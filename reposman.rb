...
optparse = OptionParser.new do |opts|
  ...
  opts.on("--addRepository PROJECT", "Identifier for project in redmine where add respository specified in -u --url and --scm") {|v| $projectIdentifier = v}
...
projects.each do |project|
  ...
  projects.each do |project|
    ...
    if project.identifier.empty?
      ...
      log("\tinvalid identifier for project #{project.name} : #{project.identifier}");
      next;
    elsif $projectIdentifier.eql? project.identifier
      if $test
        log("\tcreate repository #{$svn_url}")
        log("\trepository #{$svn_url} registered in Redmine with url #{$svn_url}") if $svn_url;
        log("\trepository #{$svn_url} created");
        next
      end

      if not $svn_url.empty?
        begin
          project.post(:repository, :vendor => $scm, :repository => {:url => "#{$svn_url}"}, :key => $api_key)
          log("\trepository #{$svn_url} registered in Redmine with url #{$svn_url}");
        rescue => e
          log("\trepository #{$svn_url} not registered in Redmine: #{e.message}");
        end
      end
